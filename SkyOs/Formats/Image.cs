...
namespace SkyOs.Formats
{
    public unsafe class Image
    {
        ...
        public Color AverageColor
        {
            get
            {
                Color T = Color.FromARGB(255, 0, 0, 0);
                for (int I = 0; I < Buffer.Length; I++)
                {
                    Color T2 = Color.FromARGB((uint)Buffer[I]);
        ...
        {
            Buffer[X + Y * Width] = (int*)Color.AlphaBlend(Color, Color.FromARGB((uint)Buffer[X + Y * Width])).ARGB;
        }
        public Color GetPixel(int X, int Y)
        {
            return Color.FromARGB((uint)Buffer[X + Y * Width]);
        }
        ...
        public Image Grayscale()
        {
            ...
            for (int I = 0; I < Buffer.Length; I++)
            {
                Color C = Color.FromARGB((uint)Buffer[I]);
                Temp.Buffer[I] = (int*)Color.FromARGB(C.A, C.R, C.R, C.R).ARGB;
            }
            return Temp;
        }
        ...
        public Image ShowChanged(Image Comparison)
        {
            ...
            for (int I = 0; I < Buffer.Length; I++)
            {
                ...
                {
                    Color C1 = Color.FromARGB((uint)Buffer[I]);
                    Color C2 = Color.FromARGB((uint)Comparison.Buffer[I]);

                    byte RDiff = (byte)Math.Abs(C1.R - C2.R);
                    byte GDiff = (byte)Math.Abs(C1.G - C2.G);
                    byte BDiff = (byte)Math.Abs(C1.B - C2.B);

                    Temp.Buffer[I] = (int*)Color.FromARGB(255, RDiff, GDiff, BDiff).ARGB;
                }
                else
                {
                    Temp.Buffer[I] = (int*)Color.Black.ARGB;
        ...
        public Image Ghost(Image Comparison)
        {
            ...
            for (int I = 0; I < Buffer.Length; I++)
            {
                Color C1 = Color.FromARGB((uint)Comparison.Buffer[I]);
                Color C2 = Color.FromARGB((uint)Buffer[I]);

                byte R = (byte)((C1.R / 2) + (C2.R / 2));
                byte G = (byte)((C1.G / 2) + (C2.G / 2));
                byte B = (byte)((C1.B / 2) + (C2.B / 2));

                Temp.Buffer[I] = (int*)Color.FromARGB(255, R, G, B).ARGB;
            }
            return Temp;
        }
        public Image GetNoise()
        {
            ...
            for (int I = 0; I < Buffer.Length; I++)
            {
                Temp.Buffer[I] = (int*)Color.FromARGB(255, (byte)Random.Next(0, 255), (byte)Random.Next(0, 255), (byte)Random.Next(0, 255)).ARGB;
            }
            return Temp;
        }
